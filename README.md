...
| `label` | no | — | Name for this device, sent as `X-Client-Label` and logged by the relay so a wake reads as `label="iPhone de Marie"` instead of a bare UUID. Provisioning it in the shared link is the point: the settings field exists to correct it, but non-technical users never open that screen. Sanitised to 24 chars of letters/digits/accents/space/`-`/`_`. |
