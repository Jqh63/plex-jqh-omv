...
**v8.1** adds a 1-tick debounce on the relay-*down* cosmetic only. A lone relay `/status` transport miss is most often a slow-but-alive relay (the e2-micro's burstable CPU is sluggish when cold) or a last-mile blip, not a dead relay — so the first miss stays optimistic (WoL button enabled, no "Relais injoignable" alarm) and only hardens if the next ~15s tick also misses. The home up/down verdict stays single-probe and immediate; and a genuine relay-down the user actually hits via WoL still surfaces instantly (the `POST /wol` failure path flips the state directly).
