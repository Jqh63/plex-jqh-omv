...
| `cold-radio-e2e.py` | Playwright headless drives the PWA on Chromium **and WebKit/Safari** (cross-browser, see § Engines) with mocked network + spoofed visibilitychange. 30 scenarios × engine. **Runtime (2026-07-29, after the parallel fan-out): ~190 s both engines, ~180 s chromium-only** — the two engines now run as one subprocess each and no longer wait for one another (`PWA_PARALLEL=0` restores the streaming sequential run) — 80 % of the family is on Android, so `PWA_ENGINES=chromium` is the iteration mode (the run prints a loud PARTIAL banner) and both engines are the merge gate. The suite drives the app on a shortened status poll (`?poll=`, test-only knob in app.js, `PWA_POLL_MS` to change it): the relay-down warn only hardens on the 3rd consecutive miss, which at the production 8 s cadence meant sampling at T+18/T+26 — cumulated wait went 158 s → 102 s per engine, same property, same positive control. ⚠️ The sentence that used to close here — "what is left is per-scenario browser overhead, not waiting" — was measured FALSE on 2026-07-29: waits are 108 s of the ~187 s, overhead 28 s. See the runtime note below. Covers the **status** machine — it never fires a wake. Since 2026-07-28 it can also take the **radio** away (`offline=True`, plus `restore_online_at_s`/`phase` to give it back): route interception answers even in an offline context, so an offline scenario whose plans still serve `up` tests a state that cannot exist — the plans must fail too, and `phase` lets them key on the radio rather than on a call count. Since 2026-07-29 it also drives the **relay-less install** (`no_relay=True`, no `&relay=` in the URL) — a fork's default, where `probe()` takes its own branch and the direct-home fetch IS the verdict; those three scenarios assert `counters['relay'] == 0`, because a relay call in that mode means the branch under test was never entered. | ~190 s both |
| `flaky-net-sim.py` | Playwright on a **latency-only** tape: the home is UP for the whole run, the relay always holds the right answer, ONLY `/status` latency varies — so every non-green second on screen is a NETWORK artefact, which is exactly the 2026-08-16 complaint. Samples the VISIBLE label 4×/s (the paint ring records decisions; this records the screen). Carries the **v8.76 grace pin**: a cold open with ONE missed probe must show zero unknown (it flashed 0,3 s before the fix — verified failing), with the 3-miss run as positive control (it MUST still reach `Statut inconnu`, else the pin would also pass on an app that never admits doubt) and a settled-verdict run pinning immunity to latency. ⚠️ Three instrument bugs were paid to get trustworthy numbers, all documented in its docstring: `time.sleep()` in a sync route handler stalls Playwright's dispatch thread (impossible timings); a relay served over `http://` is silently ignored by `validRelay()`, so the app runs its relay-LESS branch and prints a confident result about the wrong code path (the tell is `src=poll` instead of `src=hb` in the journal); and an expected label of `"Serveur"` instead of `"En ligne"` skews the whole tally. | ~190 s |
| `wake-e2e.py` | Playwright headless on the **wake** paths, which `cold-radio-e2e.py` does not touch — and where the 2026-07-14 bug lived. Pins v8.45: a wake must not survive a background freeze and repaint its countdown when the app is reopened — both for a wake this device TAPPED (`wolSent`) and, crucially, for one it merely ADOPTED from the relay (`remoteWaking`, the AM5 logon task's wake — the variant actually hit). Uses Playwright's **clock API** to reproduce the Android freeze/thaw. Two traps it exists to avoid, both of which produced a green-but-worthless test on the first pass: for the **reap** scenarios assert on the countdown (`powerProgress`), not the status card; and jump time with `set_system_time`, **not** `fast_forward` (the latter also fires the thawed poll timer, which reaps the wake on its own — the test would pass even without the fix). ⚠️ v8.53 — that first trap was half a misreading: the card being "repainted to Vérification… while the countdown keeps ticking" was not only a fixture artefact, it was a **real defect** on adopted wakes (`setRechecking` guarded `wolSent` but not `remoteWaking`). `remote-wake-outlives-the-relay-waking-signal` now asserts on the card ON PURPOSE. It also has to **sample** across the ~2.5 s (`DOWN_RECHECK_MS`) contradiction window rather than snapshot once — a single late snapshot passes against the bug. ⚠️ v8.72 — `degraded-resume-does-not-paint-green-early` replays the 2026-08-03 IRL bug (green 29 s early, then stuck): the wake hold cached a bare `up`, and a resume pre-painted that cache through `setOnline()`, which also **ends the wake**. The scenario needs **no time travel** — the wake is live and must stay so; what it needs is a poll *after* the resume (the assertion that catches the second half: green that came to stay, not just early) and the ready-at-the-end positive control, without which "never go green" would pass. ⚠️ v8.77 — `late-adopted-wake-still-shows-a-timer` covers the structural window nothing else did: the relay advertises `waking` for 150 s while the ETA is ~80 s, so a PWA opened cold in that 70 s tail adopts a wake **older than the ETA**. `startCountdown` clamped the anchor to the ETA, which froze the label ("démarrage en cours sans timer", reported 2026-08-17). The assertion is that the label **moves** between two samples — a frozen widget reads as a hung app — with an early adopter as positive control, without which "the label moves" would pass on an app that ticks meaningless strings. ⚠️ The truncation of that longer label is **not** covered by `text-overflow-e2e.py` (it audits tile labels/subs, never `powerLabel`), and the obvious `scrollWidth > clientWidth` predicate is **inert** here: the label is not width-constrained, so a 432 px string in a 256 px viewport reports "ok" while overflowing the screen. Measure the bounding rect against the viewport instead. | ~85 s |
...
| `text-overflow-e2e.py` | **Truncation audit** of every user-visible string: each tile label/sub at 256/280/300/320/360/384/412 px CSS, plus the toasts. Reported by Yann 2026-07-28 ("les sous-titres souvent coupés") and found at **320 px** — the width an Android phone reaches on the 'large' *display size* setting, which the earlier 360 px pins never covered. Asserts `scrollWidth > clientWidth` (the DOM text still reads fine, so nothing else notices) and, for toasts, more than two lines. Two traps met writing it: `getBoundingClientRect().height` includes padding, so dividing it by the line height reported 3 lines for a 4-word toast — measure against a one-line baseline in the same box; and a font-scale axis was **inert**, because all 32 font sizes in `index.html` are `px`, never `rem` (which is itself a finding: Android's text-size setting does nothing here — but the family uses the *display size* slider, which divides the CSS width instead, hence the 256/280 columns). Also pins that the <=300 px wrap does not move the power button, **with a positive control** (a deliberately 3-line sub must move it, or the pin measures nothing). | ~15 s |
| `version-footer-e2e.py` | Pins the version label the family SEES (`#footerVersion`) and the `version.js` parser behind it. Two layers: parser cases evaluated in the page (the update beat where the OLD and NEW caches coexist cannot be staged live), and the **rendered** footer over `http://` — a service worker needs a real origin, `file://` cannot register one. Exists because the footer had zero coverage: asked whether the version was visible anywhere, I grepped, answered "nowhere in the UI", and was wrong. The render layer immediately caught a real pre-existing bug the parser layer could not see — on a FIRST visit the cache does not exist yet when the page reads it, so the footer stayed `—` until the next launch. | ~10 s |
...
| `settings-field-states-e2e.py` | Render pins for the **state** of the settings fields (2026-09-07): a field the family cannot edit must not paint like one they can. `cfgWindow` had been correctly locked since v8 and the hint said so, but the sheet carried a `:disabled` rule for `.test-btn` and **none** for `.field input`. Measured on both engines BEFORE writing the fix: normal / disabled / readonly computed identical on Chromium *and* WebKit — never a per-engine default, so engine-sniffing would have found nothing. Pins the relay-owned window as `readonly` (not `disabled`: the value must stay selectable and in the accessibility tree), the token + test button as inert with no relay URL and live again on the keystroke that provides one, and an in-flight test surviving a re-render.  Since 2026-09-07 it also sweeps the settings screen at **256/280/300/320/360/412/1280 px** on both engines, with an armed positive control: the 16px bump made every field wider, and `text-overflow-e2e.py` audits tile labels and toasts — **never this screen** — so its green proved nothing about Android's *display size* range or desktop Chrome. Two traps met writing it: the distinctness rows first included `cursor`, which **passes against the unfixed code** (a disabled input gets a different UA cursor with no rule at all) and is invisible on the phones the family actually uses — assert on opacity/border/background only; and a missing selector or helper aborted the whole run before the second engine, so every row now degrades to a failure instead. | ~10 s |
...
> **Where the runtime actually goes** (measured 2026-07-29 with `PWA_TIMING=1`,
> after two confident wrong guesses — browser launch is 0.15 s, not a
> bottleneck, and there is no hidden cost outside the scenarios). Per engine:
> 108 s of fixed sample waits across 27 scenarios, 28 s of browser/context/goto
> overhead, 51 s for the four special runners (resume ×2, clockjump, watchdog)
> — ~187 s, against a 190 s wall clock. **The time IS the waiting**, and that
> waiting encodes real timing properties (`DOWN_RECHECK_MS`, the 3-miss relay
> debounce, `CHECK_WATCHDOG_MS`): shortening it would weaken what the scenarios
> prove. What was free to remove is the engines queueing behind each other,
> hence the fan-out. Going below ~190 s means running SCENARIOS concurrently,
> which needs `collect_results()` restructured so every scenario is launched
> before any assertion reads it — not attempted, and not worth it until the
> suite grows.
...
> ⚠️ **`PWA_BASE` defaults to the WORKING TREE** (`file://` on this checkout),
> so both E2E suites are a *pre*-merge gate. This was flipped on 2026-07-19: the
> old default was the deployed app, and a green run proved nothing about
> uncommitted changes (it bit us on 2026-07-18 — new scenarios "passed" against
> the live v8.47, then again with a deterministic v8.49 regression read as
> "pre-existing flaky"). To gate the live deploy *after* a merge, opt in
> explicitly: `PWA_BASE=deployed python3 tests/…` (or any explicit URL).
...
# expect: [chromium] ALL PASS (29 scenarios)  /  ALL ENGINES PASS
```
