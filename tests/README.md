...
| `cold-radio-e2e.py` | Playwright headless drives the PWA on Chromium **and WebKit/Safari** (cross-browser, see § Engines) with mocked network + spoofed visibilitychange. 29 scenarios × engine. **Runtime (2026-07-29): ~370 s both engines, ~180 s chromium-only** — 80 % of the family is on Android, so `PWA_ENGINES=chromium` is the iteration mode (the run prints a loud PARTIAL banner) and both engines are the merge gate. The suite drives the app on a shortened status poll (`?poll=`, test-only knob in app.js, `PWA_POLL_MS` to change it): the relay-down warn only hardens on the 3rd consecutive miss, which at the production 8 s cadence meant sampling at T+18/T+26 — cumulated wait went 158 s → 102 s per engine, same property, same positive control. What is left is per-scenario browser overhead, not waiting. Covers the **status** machine — it never fires a wake. Since 2026-07-28 it can also take the **radio** away (`offline=True`, plus `restore_online_at_s`/`phase` to give it back): route interception answers even in an offline context, so an offline scenario whose plans still serve `up` tests a state that cannot exist — the plans must fail too, and `phase` lets them key on the radio rather than on a call count. Since 2026-07-29 it also drives the **relay-less install** (`no_relay=True`, no `&relay=` in the URL) — a fork's default, where `probe()` takes its own branch and the direct-home fetch IS the verdict; those three scenarios assert `counters['relay'] == 0`, because a relay call in that mode means the branch under test was never entered. | ~30 s/engine |
